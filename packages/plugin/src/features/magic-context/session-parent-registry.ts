 ...
 *
 * Child sessions (dreamer, user task subagents) execute tools with
 ...
 *     root session.
 *   - ctx_search's "already visible" memory filter (memory_block_ids): it is
 *     persisted on the root session's session_meta; reading it by the child id
 *     silently re-surfaces content the parent conversation already shows.
 *
