...
- \`action="verify", ids=[N]\` — confirm a memory is still correct (records verification, refreshes long-term memory recency)
...
3. **For each verifiable memory:**
   ...
   - If the memory is correct: \`ctx_memory(action="verify", ids=[N])\` — records the verification and refreshes long-term memory recency
