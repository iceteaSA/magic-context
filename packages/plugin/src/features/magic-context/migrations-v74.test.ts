...
describe("migration v74 — skill_memory embeddings + FTS", () => {
    ...
    test("v74 migration backfills FTS for rows that pre-existed v40", () => {
        const db = new Database(":memory:");
        try {
            initializeDatabase(db);
            ...
            // Insert a row under the pre-v74 schema — no FTS table yet, so no AFTER-INSERT trigger indexes it.
            db.prepare(
                `INSERT INTO skill_memory
                   (skill_id, resolved_path, tier, project_identity, intent, kind, delta, normalized_hash, hit_count, pinned, created_at)
                 VALUES (?,?,?,?,?,?,?,?,0,0,?)`,
            ...
            if (!v74) throw new Error("v40 migration not found");
            v74.up(db);
            const hit = db
                .prepare(
