...
describe("promoteSkillObservations", () => {
    test("direct-writes a global '*' note with historian provenance", () => {
        ...
        try {
            ...
            const row = db
                .prepare(
                    "SELECT tier, project_identity, origin_project, source_type, resolved_path, kind FROM skill_memory",
                )
    ...
    test("exact-hash duplicate bumps hit_count instead of inserting", () => {
        ...
        try {
            promoteSkillObservations(db, "git:repoA", [
                { skillId: "council", kind: "fix", lesson: "same lesson" },
            ]);
            const n = promoteSkillObservations(db, "git:repoB", [
                { skillId: "council", kind: "fix", lesson: "same lesson" },
            ]);
            expect(n).toBe(0);
            const rows = db.prepare("SELECT hit_count FROM skill_memory").all() as Array<{
                hit_count: number;
            }>;
    ...
    test("rejects kind='general'", () => {
        ...
        try {
            const n = promoteSkillObservations(db, "git:repoA", [
                { skillId: "council", kind: "general" as never, lesson: "x" },
            ]);
            expect(n).toBe(0);
        } finally {
            closeQuietly(db);
