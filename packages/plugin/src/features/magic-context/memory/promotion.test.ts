...
describe("promotion", () => {
    ...
    describe("#given external memory tee", () => {
        it("tees newly inserted facts with project scope", async () => {
            ...
            const { newMemoryRefs } = promoteSessionFactsDurable(db, "ses_1", "git:rootsha", [
                { category: "PROJECT_RULES", content: "tee me" },
            ]);
            await embedPromotedFacts(db, "ses_1", "git:rootsha", newMemoryRefs, {
                projectName: "myproj",
            });
            await Bun.sleep(10);
        ...
        it("does NOT tee dedup hits", async () => {
            ...
            const first = promoteSessionFactsDurable(db, "ses_1", "git:rootsha", [
                { category: "PROJECT_RULES", content: "dup fact" },
            ]);
            await embedPromotedFacts(db, "ses_1", "git:rootsha", first.newMemoryRefs);
            await Bun.sleep(10);
            const second = promoteSessionFactsDurable(db, "ses_2", "git:rootsha", [
                { category: "PROJECT_RULES", content: "dup fact" },
            ]);
            await embedPromotedFacts(db, "ses_2", "git:rootsha", second.newMemoryRefs);
            await Bun.sleep(10);
