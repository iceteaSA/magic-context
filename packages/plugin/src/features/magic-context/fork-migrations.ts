...
export const FORK_MIGRATIONS: Migration[] = [
...

    {
        ...
        up: (db: Database) => {
            // skill_memory is migration-only (created by fork migration 10_000); ALTER is safe here.
 ...
 */
/**
 * Retry wrapper for the fork lane, mirroring upstream's `runMigrationsWithRetry`.
 *
 * The async open path retries the upstream pass so a sibling instance holding the
 * write lock during boot does not fail the open. The fork pass runs immediately
 * after and can hit the same contention, so it needs the same discipline —
 * without this, `runForkMigrations` raising `MigrationLockBusyError` escapes as a
 * hard open failure and the plugin fails closed on a lock that would have cleared.
 */
export async function runForkMigrationsWithRetry(
    db: Database,
    options: { retryDelaysMs?: number[]; sleep?: (delayMs: number) => Promise<void> } = {},
): Promise<void> {
    ...
    for (let attempt = 1; attempt <= totalAttempts; attempt += 1) {
        try {
            ...
            log(
                `[migrations] fork lane write lock is busy; retrying attempt ${attempt + 1}/${totalAttempts} in ${delayMs}ms`,
            );
            await sleep(delayMs);
        }
    }
...
export function runForkMigrations(db: Database): void {
    ...
    // Read-first gate. The per-migration applied-check also runs inside the
    // immediate transaction below (double-checked locking, for the sibling race),
    // but checking here first means a fully-migrated database takes NO write lock
    // at all. Without this the steady-state open grabs the write lock on every
    // boot and fails whenever another instance holds it — upstream's runner reads
    // its version watermark before locking, and the fork lane must match.
    const pendingVersions = new Set(
        ordered
            .filter((migration) => !isForkMigrationApplied(db, migration.version))
            .map((migration) => migration.version),
    );
    if (pendingVersions.size === 0) return;
    log(`[migrations] fork lane: applying ${pendingVersions.size} downstream migration(s)`);
