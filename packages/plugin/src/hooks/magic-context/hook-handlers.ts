...
export function createToolExecuteAfterHook(args: {
    ...
    /** Skill-memory wiring. OPTIONAL as a group: the v1 plugin host supplies all
     *  four, while opencode2's `context.tool.hook("execute.after")` path does not
     *  wire skill-memory at all. When any of them is absent the skill branch below
     *  is skipped and every other duty (ignored-message flush, Channel 1, todo
     *  capture) still runs, so an unwired host degrades to upstream behaviour
     *  instead of throwing. */
    ...
    // Narrowed once into a single object so the branch below has provably-defined
    // members; a boolean flag would not narrow the optional fields on `args`.
    const skillMemory =
        args.skillLoadRegistry !== undefined &&
        args.sessionDirectoryBySession !== undefined &&
        args.defaultDirectory !== undefined &&
        args.intentByCallId !== undefined
            ? {
                  registry: args.skillLoadRegistry,
                  sessionDirectoryBySession: args.sessionDirectoryBySession,
                  defaultDirectory: args.defaultDirectory,
                  intentByCallId: args.intentByCallId,
              }
            : undefined;
    return async (input: unknown, output?: unknown) => {
        ...
        if (typedInput.tool === "skill" && skillMemory) {
            ...
            if (typeof typedOutput?.output === "string") {
                ...
                if (skillId) {
                    ...
                    const mappedDir = skillMemory.sessionDirectoryBySession.get(
                        typedInput.sessionID,
                    );
                    const sessionDir = mappedDir ?? skillMemory.defaultDirectory;
                    try {
                        ...
                        if (provenance) {
                            ...
                            skillMemory.registry.set(registryKey(typedInput.sessionID, skillId), {
                                ...provenance,
                                frontmatterConfig,
                            });
                        }
                    ...
                    try {
                        const registryEntry = skillMemory.registry.get(
