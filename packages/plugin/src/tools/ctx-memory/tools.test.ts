...
describe("createCtxMemoryTools", () => {
    ...
    describe("#given corrective propagation to external backend", () => {
        ...
        it("archive action propagates remove to external backend", async () => {
            ...
            // A dreamer archive is gated on an ACTIVE same-category successor
            // (curate safety, `missing-active-same-category-successor`), so the
            // consolidation target has to exist before the archive is attempted.
            const successorResult = await tools.ctx_memory.execute(
                {
                    action: "write",
                    category: "PROJECT_RULES",
                    content: "consolidated replacement fact",
                },
                toolContext("ses-dreamer", DREAMER_AGENT),
            );
            const successorId = extractMemoryId(successorResult);
