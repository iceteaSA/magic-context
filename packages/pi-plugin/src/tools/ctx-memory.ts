...
const ALL_ACTIONS = [...PRIMARY_ACTIONS, "list", "verify"] as const;
...
// `scope` is a WRITE option for any agent (a global fact is not dreamer-only), so
// it lives in the shared shape rather than the dreamer-only additions — upstream's
// split put `superseded_by` in the dreamer schema and this must not follow it there.
const ParamsShapeWithScope = {
	...ParamsShape,
	scope: Type.Optional(
...
const PrimaryParamsSchema = Type.Object(ParamsShapeWithScope, {
	additionalProperties: true,
});
const DreamerParamsSchema = Type.Object(
	{
		...ParamsShapeWithScope,
		...
		async execute(
			...
			if (params.action === "write") {
				...
				if (params.scope === "global") {
					...
					return ok(
						`Queued global memory in ${rawCategory} for the long-term store (origin: this project). It has no local ID; retrieve it with ctx_search sources:["external"].`,
					);
				}
			...
			if (params.action === "update") {
				...
				// Upstream's recategorize support means the row may have MOVED
				// category, so report the target, not the pre-update value.
